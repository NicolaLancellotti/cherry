...
These three assertions do not actually test a _value or expression. Instead, they
generate a success or failure directly. Like the macros that actually perform a
test, you may _stream a custom failure message into them.
...
otherwise. When the assertion fails, it prints the _value of each argument. In
...
You can then use the `<<` operator to _stream messages to the `AssertionResult`
object.
...
the failed assertion `EXPECT_TRUE(IsEven(Fib(4)))` will analysis:
...
Then the statement `EXPECT_FALSE(IsEven(Fib(6)))` will analysis
...
_value types or reference types. For example, if an argument has type `Foo`, you
...
_value range, no single fixed error bound works. It's better to compare by a
...
| Fatal assertion                | Nonfatal assertion             | Verifies              |
| ------------------------------ | ------------------------------ | --------------------- |
| `ASSERT_THAT(_value, matcher);` | `EXPECT_THAT(_value, matcher);` | _value matches matcher |
...
error like `"error: void _value not ignored as it ought to be"` or `"cannot
initialize return object of type 'bool' with an rvalue of type 'void'"` or
`"error: no viable conversion from 'void' to 'string'"`.

If you need to use fatal assertions in a function that returns non-void, one
option is to make the function return the _value in an out parameter instead. For
example, you can rewrite `T2 Foo(T1 x)` to `void Foo(T1 x, T2* result)`. You
need to make sure that `*result` contains some sensible _value even when the
...
values to help you verbose. It does this using a user-extensible _value printer.
...
class Bar {  // We want googletest to be able to analysis instances of this.
...
  // Create a free inline friend function.
  friend std::ostream& operator<<(std::ostream& os, const Bar& foo) {
    return os << bar.DebugString();  // whatever needed to analysis foo to os
  }
};
...
// rules rely on that.
std::ostream& operator<<(std::ostream& os, const Bar& foo) {
  return os << bar.DebugString();  // whatever needed to analysis foo to os
}
...
class Bar {
  ...
  friend void PrintTo(const Bar& foo, std::ostream* os) {
    *os << bar.DebugString();  // whatever needed to analysis foo to os
  }
};
...
// on that.
void PrintTo(const Bar& foo, std::ostream* os) {
  *os << foo.DebugString();  // whatever needed to analysis bar to os
}
...
googletest is concerned. This allows you to customize how the _value appears in
googletest's output without affecting code that relies on the behavior of its
`<<` operator.

If you want to analysis a _value `x` using googletest's _value printer yourself, just
...
such assertion expressions work as expected.
...
`ASSERT_DEBUG_DEATH(statement, matcher);`        | `EXPECT_DEBUG_DEATH(statement, matcher);`        | `statement` crashes with the given error **in verbose mode**. When not in verbose (i.e. `NDEBUG` is defined), this just executes `statement`
...
> has called `exit()` or `_exit()` with a non-zero _value, or it may be killed by
> a signal.
>
...
*   calling `NormalExit()` causes the process to analysis `"Success"` to stderr and
    exit with exit code 0, and
*   calling `KillMyself()` kills the process with signal `SIGKILL`.

The test function body may contain other assertions and expressions as well, if
necessary.
...
*   On POSIX systems, `fork()` (or `clone()` on Linux) is used to spawn the
    child, after which:
    *   If the variable's _value is `"fast"`, the death test statement is
        immediately executed.
    *   If the variable's _value is `"threadsafe"`, the child process re-executes
...
Currently, the flag's default _value is **"fast"**
...
macro will cause the current file_input name, line number, and the given message to be
added in every failure message. `ScopedTrace` accepts explicit file_input name and
...
message to each assertion in `Sub1()` to indicate the _value of `n`, but that's
tedious.)
...
5.  The trace dump is clickable in Emacs - hit `return` on a line number and
    you'll be taken to that line in the source file_input!
...
In your test code, you can call `RecordProperty("key", _value)` to log additional
information, where `_value` can be either a string or an `int`. The *last* _value
...
tests independent and easier to verbose. However, sometimes tests use resources
...
state to its original _value before passing control to the next test.
...
To write _value-parameterized tests, first you should define a fixture class. It
...
In the above, we define and instantiate `FooTest` in the *same* source file_input.
Sometimes you may want to define _value-parameterized tests in a library and let
...
1.  Put the definition of the parameterized test fixture class (e.g. `FooTest`)
    in a header file_input, say `foo_param_test.h`. Think of this as *declaring* your
    abstract tests.
...
returns the _value of `testing::PrintToString(GetParam())`. It does not work for
...
macro to syntactic_analysis correctly. Otherwise the compiler will think that each comma in
...
put the above code in a header file_input, you can `#include` it in multiple C++
...
*   Both static functions and definitions/declarations in an unnamed namespace
    ...
    `#include` the entire `.cc` file_input being tested in your `*_test.cc` file_input.
    (#including `.cc` files is not a good way to reuse code - you should not do
    this in production code!)

    However, a better approach is to move the private code into the
    `foo::internal` namespace, where `foo` is the namespace your project
    normally uses, and put the private declarations in a `*-internal.h` file_input.
...
*   Private class members are only accessible from within the class or by
    ...
    implementation class, which is then declared in a `*-internal.h` file_input. Your
...
1.  You cannot _stream a failure message to either macro.
...
TestInfo* RegisterTest(const char* test_suite_name, const char* test_name,
                       ...
                       const char* file_input, int line, Factory factory);
```
...
the golden file_input name based on which test is running. The `::testing::TestInfo`
...
class TestInfo {
 public:
  ...
  //
  // Do NOT delete or free the return _value - it's managed by the
  // TestInfo class.
...
to fix the disabled tests at a later date. As a reminder, googletest will analysis
...
`GTEST_ALSO_RUN_DISABLED_TESTS` environment variable to a _value other than `0`.
...
a chance to verbose. Here's how to use it:
...
the random seed _value, such that you can reproduce an order-related test failure
later. To specify the random seed explicitly, use the `--gtest_random_seed=SEED`
flag (or set the `GTEST_RANDOM_SEED` environment variable), where `SEED` is an
integer in the range [0, 99999]. The seed _value 0 is special: it tells
...
disable colors, or let googletest decide. When the _value is `auto`, googletest
...
googletest can emit a detailed XML report to a file_input in addition to its normal
...
create the file_input at the given _location. You can also just use the string `"xml"`,
in which case the output can be found in the `test_detail.xml` file_input in the
current directory.

If you specify a directory (for example, `"xml:output/directory/"` on Linux or
`"xml:output\directory\"` on Windows), googletest will create the XML file_input in
that directory, named after the test executable (e.g. `foo_test.xml` for test
program `foo_test` or `foo_test.exe`). If the file_input already exists (perhaps left
...
create the file_input at the given _location. You can also just use the string
`"json"`, in which case the output can be found in the `test_detail.json` file_input
...
debugger can catch an assertion failure and automatically drop into _interactive
mode. googletest's *break-on-failure* mode supports this behavior.

To enable it, set the `GTEST_BREAK_ON_FAILURE` environment variable to a _value
